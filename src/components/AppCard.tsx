...
<div className="px-4 pb-3 pt-4 border-b border-violet-200 bg-violet-100/20 flex items-center justify-between">
        ...
        <Link href={'https://taricov.daftra.com/owner/staff/'+ d.sales_person_id} target="_blank" className="sm:text-sm text-xs flex items-center hover:underline transition">
        <span className="line-clamp-1 md:line-clamp-0">
            {d.sales_person_id === 0 ? "Admin" : d.sales_person > 0 ? "No Sales Person" : "Admin"}
        </span>
...
<div className="transition duration-100 text-gray-500 hover:text-gray-500/80 cursor-pointer">
    <Link href={"http://taricov.daftra.com/owner/invoices/view/"+d.id} target="_blank">
...
<svg xmlns="http://www.w3.org/2000/svg" width="20" viewBox="0 0 24 24"><path fill="currentColor" d="M10 17c0-3.31 2.69-6 6-6h3V5h-2v3H7V5H5v14h5v-2z" opacity=".3"/><path fill="currentColor" d="M10 19H5V5h2v3h10V5h2v6h2V5c0-1.1-.9-2-2-2h-4.18C14.4 1.84 13.3 1 12 1s-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h5v-2zm2-16c.55 0 1 .45 1 1s-.45 1-1 1s-1-.45-1-1s.45-1 1-1z"/><path fill="currentColor" d="m18.01 13l-1.42 1.41l1.58 1.58H12v2h6.17l-1.58 1.59l1.42 1.41l3.99-4z"/></svg>
    ...
    <div className="w-full px-4 pb-3 pt-4 border-b border-gray-300 bg-gray-100 flex justify-between">
        <div className="text-xs uppercase font-bold text-gray-600 tracking-wide">Invoice: <div className="font-normal">{d.no}</div></div>
        <div className="text-right text-xs uppercase font-bold text-gray-600 tracking-wide">Payment Status: <div className={`font-normal ${paymentsStatusColors[d.payment_status === 2 ? "paid" : d.payment_status === 1 ? "unpaid" : "partially paid"]} bg-clip-text text-transparent`}>{d.payment_status === 2 ? "paid" : d.payment_status === 1 ? "unpaid" : "partially paid"}</div></div>
    </div>
    <div className="p-4 w-full text-gray-700 flex justify-between items-start">
        <div>
            <p className="text-3xl text-gray-900 leading-none my-1">{d.summary_total + " " + d.currency_code}</p>
            <p className="text-xs w-full flex items-center">
                <span className=''>
                    Client:&nbsp;
                </span>
                <Link target="_blank" href={'https://'+  "taricov" +'.daftra.com/owner/clients/view/'+d.clients.id} className="sm:text-sm text-xs flex items-center hover:underline transition">
        <span className="line-clamp-1 md:line-clamp-0">
        ...
        <div className="leading-loose bg-gray-500/10 text-gray-900/50 p-1 px-2 rounded-lg uppercase text-xs tracking-wider">{d.clients.follow_up_status || "No Status"}</div>
    </div>
    <div className="flex justify-around items-center text-center p-4 border-t border-gray-300 text-gray-600">
            <p className="">
                ...
                <span className="block text-gray-900 font-bold">{d.summary_discount}</span>
            </p>

            <p className="">
                <span className="text-sm pr-1">Tax</span>
                <span className="block text-gray-900 font-bold">{d.tax1}</span>
            </p>
            <p className="">
